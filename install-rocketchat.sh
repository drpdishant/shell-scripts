...
if ls $HOME/Downloads/rocketchat* 1> /dev/null 2>&1; then
    ...
    wget -qO https://github.com/RocketChat/Rocket.Chat.Electron/releases/download/$latest/rocketchat_${latest}_amd64.deb -P ~/Downloads
fi
...
echo -e "Checking if it can be updated\n"
    ...
    wget -qO https://github.com/RocketChat/Rocket.Chat.Electron/releases/download/$latest/rocketchat_$latest_amd64.deb -P ~/Downloads
    sudo dpkg -i ~/Downloads/rocketchat*
    sudo apt-get -y -qq -f install  
