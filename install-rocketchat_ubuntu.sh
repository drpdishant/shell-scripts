...
if ls $HOME/Downloads/rocketchat* 1> /dev/null 2>&1; then
    ...
    cd ~/Downloads && { curl -O https://github.com/RocketChat/Rocket.Chat.Electron/releases/download/$latest/rocketchat_${latest}_amd64.deb; cd -; }
...
echo -e "Checking if it can be updated\n"
    ...
    cd ~/Downloads && { curl -O https://github.com/RocketChat/Rocket.Chat.Electron/releases/download/$latest/rocketchat_$latest_amd64.deb; cd -; }
    sudo dpkg -i ~/Downloads/rocketchat*
    sudo apt-get -y -qq -f install  
